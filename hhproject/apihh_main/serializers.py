...
class UserProfileSerializer(serializers.ModelSerializer):
    ...
    def validate_email(self, value):
        ...
        current_email = _normalize_email_value(getattr(self.instance, 'email', ''))
        if current_email and value == current_email:
            return value

        raise serializers.ValidationError(
            'Для смены email используйте подтверждение кода, отправленного на новую почту.'
        )
...
class ProfileEmailChangeRequestSerializer(serializers.Serializer):
    ...
    def validate_email(self, value):
        ...
        current_email = _normalize_email_value(getattr(user, 'email', ''))
        if value == current_email:
            raise serializers.ValidationError('Укажите новый email, отличный от текущего.')
...
class ProfileEmailChangeConfirmSerializer(ProfileEmailChangeRequestSerializer):
